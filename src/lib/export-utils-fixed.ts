 ...
 */
export function transformInvoiceData(invoices: Invoice[]): Record<string, Record<string, unknown>[]> {
  const electricityData: Record<string, unknown>[] = [];
  const gasData: Record<string, unknown>[] = [];
  const waterData: Record<string, unknown>[] = [];
  ...
  invoices.forEach(invoice => {
    ...
    const transformedData: Record<string, unknown> = { ...baseData };
